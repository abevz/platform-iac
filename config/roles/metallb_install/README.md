...
metallb_ip_range: "192.0.2.200-192.0.2.220"
```
...
- name: Production Cluster with Large IP Pool
  ...
  become: yes
  vars:
    metallb_ip_range: "192.0.2.100-192.0.2.200"
  roles:
    - metallb_install
...
spec:
  addresses:
  - 192.0.2.200-192.0.2.220  # IP range
```
...
# nginx-lb   LoadBalancer   10.96.123.45    192.0.2.200    80:30123/TCP
```
...
spec:
  type: LoadBalancer
  loadBalancerIP: 192.0.2.205  # Request specific IP from pool
  selector:
    app: myapp
  ports:
  - port: 443
...
spec:
  type: LoadBalancer
  loadBalancerIP: 192.0.2.210  # Same IP as http-service
  ports:
  - port: 443
    targetPort: 8443
```
...
spec:
  myASN: 64512
  peerASN: 64512
  peerAddress: 192.0.2.1  # Router IP
---
apiVersion: metallb.io/v1beta1
