...
load_tofu_secrets_to_temp_file() {
  ...
  if [ "$COMPONENT" == "nginx-proxy" ] || [ "$COMPONENT" == "minio" ]; then
    log "ВНИМАНИЕ: Bootstrap-компонент. Переопределяем proxmox_api_url на прямой IP (192.0.2.101)."
    # (IP взят из Вашего конфига nginx для homelab.example.com)
    PROXMOX_ENDPOINT="https://192.0.2.101"
  else
