...
Task tracking goes through `dibs` — Beads (`bd`) is decommissioned, do
not use it. The protocol lives in the managed block below; update it by
re-running `dibs init` after upgrading `dibs`. Use your stable tool
...
1. File issues for remaining work (`dibs issue create`)
